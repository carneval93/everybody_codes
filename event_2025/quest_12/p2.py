...
def breadth_search(start_node, end_node, v_nodes):
    breadth_nodes = list()
    breadth_nodes.append(start_node)
    breadth_nodes.append(end_node)
    while breadth_nodes:
...
print(len(breadth_search((0,0), (len(inp)-1, len(inp[0])-1),[(0,0), (len(inp)-1, len(inp[0])-1)])))