...
for segment in inp:
    ...
    for i in range(segment_list[1]):
        if (segment_list[0], i) not in hole:
            wall.add((segment_list[0], i))
    for i in range(segment_list[1], segment_list[1] + segment_list[2]):
        hole.add((segment_list[0], i))
    for i in range(segment_list[1] + segment_list[2], 1000):
        if (segment_list[0], i) not in hole:
            wall.add((segment_list[0], i))
