class Node:
    ...
    def attach_new_node(self, new_node):
        if self.visited_left and self.visited_right and self.get_id() != 1:
            ...
            if next_left and self.get_leftSocket_color() == new_node.get_plug_color() and self.get_leftSocket_shape() == new_node.get_plug_shape() and self.get_left_bound() == 'WEAK':
                self.set_left_node(new_node)
                new_node.set_plug_node(self)
                self.set_left_bound('STRONG')
                self.attach_new_node(next_left)
            elif not next_left and self.get_leftSocket_color() == new_node.get_plug_color() and self.get_leftSocket_shape() == new_node.get_plug_shape():
                self.set_left_node(new_node)
                new_node.set_plug_node(self)
                self.set_left_bound('STRONG')
            ...
            if next_right and self.get_rightSocket_color() == new_node.get_plug_color() and self.get_rightSocket_shape() == new_node.get_plug_shape() and self.get_right_bound() == 'WEAK':
                self.set_right_node(new_node)
                new_node.set_plug_node(self)
                self.set_right_bound('STRONG')
                self.attach_new_node(next_right)
            elif not next_right and self.get_rightSocket_color() == new_node.get_plug_color() and self.get_rightSocket_shape() == new_node.get_plug_shape():
                self.set_right_node(new_node)
                new_node.set_plug_node(self)
                self.set_right_bound('STRONG')
